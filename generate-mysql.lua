...
local sourceDb, user = 'snomed', 'root'
local sourceTable = 'curr_description_fairseq'
local dest_field = 'term_fa_en_fr'
local source_field = 'term_en'
local source_field_no = 5
local limit = 2
local counterStep = 1000
local currentId
local counter = 0
...
local conn = env:connect(sourceDb, user, passw)
conn:execute('SET NAMES utf8')
local cursor = conn:execute('SELECT * FROM ' .. sourceTable .. ' WHERE 1 LIMIT ' .. limit)
...
TextFileIterator.__init = argcheck{
    ...
    call = function(self, path, transform)
        function self.run()
            ...
            return function()
                -- Get data from MYSQL cursor
                local data = {cursor:fetch()}
                if data[1] then
                  currentId = data[1]
                  counter = counter + 1
                  if counter - math.floor(counter/counterStep)*counterStep == 0 then
                    print('Counter', counter)
                  end
                  return transform(data[source_field_no])
                else
                  fd:close()
                end
...
for sample in dataset() do
    ...
    for i = 1, math.min(config.nbest, config.beam) do
        ...
        local command = 'UPDATE ' .. sourceTable .. ' SET ' .. dest_field .. '="' .. hypo .. '" WHERE id=' .. currentId
        -- print('hypo', hypo, 'currentId', currentId, command)
        local insert = conn:execute(command)
